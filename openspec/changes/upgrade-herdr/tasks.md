...
- [ ] 1.1 Update `inputs.herdr.url` in `flake.nix` from `github:herdrdev/herdr/v0.8.2` to `github:herdrdev/herdr/v0.9.0`
- [ ] 1.2 Run `nix flake lock --update-input herdr` to regenerate the `flake.lock` entry

## 2. Build and Compatibility Verification

- [ ] 2.1 Run `make home-build` to confirm the herdr package and the `herdrAgentFiles` derivation build against v0.9.0
- [ ] 2.2 Validate the config baseline against the v0.9.0 binary: build the config from `home-manager/herdr.nix` and run `herdr config check` on it, confirming `one-dark` is still accepted as a built-in theme name and `ui.toast.delivery = "system"` is valid
...
- [ ] 3.2 After switch, run `herdr --version` to confirm v0.9.0 is on PATH and `herdr config check` passes on the live `~/.config/herdr/config.toml`
