...
- **WHEN** the derivation is built with a valid `rev` (e.g., `v1.8.0`)
...
- **THEN** `.pi/prompts/` contains exactly 12 files: `opsx-apply.md`, `opsx-archive.md`, `opsx-bulk-archive.md`, `opsx-continue.md`, `opsx-explore.md`, `opsx-ff.md`, `opsx-new.md`, `opsx-onboard.md`, `opsx-propose.md`, `opsx-sync.md`, `opsx-update.md`, `opsx-verify.md`
...
- **WHEN** the build completes
- **THEN** `.opencode/commands/` contains exactly 12 files matching the same workflow IDs as pi prompts (including `opsx-update.md`)
...
- **WHEN** the build completes
- **THEN** `.opencode/skills/` contains exactly 12 subdirectories matching the same names as pi skills (including `openspec-update-change`)
...
- **WHEN** the derivation version is updated to v1.8.0 and `make home-build` is run
- **THEN** all file paths referenced in `home-manager/openspec.nix` (including the four new `update` workflow entries) still resolve in the derivation output
