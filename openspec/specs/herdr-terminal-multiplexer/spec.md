...
The system SHALL declare herdr as a flake input at `inputs.herdr.url = "github:herdrdev/herdr/v0.8.0"` and SHALL NOT override its nixpkgs input via `follows`.

#### Scenario: Herdr flake input is declared

- **WHEN** the flake is evaluated (`nix flake check` or `nix flake show`)
- **THEN** the `herdr` input resolves to the `github:herdrdev/herdr` repository at tag `v0.8.0`
